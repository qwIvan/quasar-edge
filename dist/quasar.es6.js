...
var Numeric = {render: function(){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('div',{staticClass:"q-numeric textfield row inline items-center",class:{disabled: _vm.disable, readonly: _vm.readonly, 'has-error': _vm.hasError}},[_c('i',{on:{"click":function($event){_vm.__setByOffset(-1);}}},[_vm._v("remove")]),_vm._v(" "),_c('input',{directives:[{name:"model",rawName:"v-model.number",value:(_vm.model),expression:"model",modifiers:{"number":true}}],staticClass:"no-style auto q-input-field",style:({width: _vm.width}),attrs:{"type":"number","pattern":"[0-9]*","disabled":_vm.disable,"readonly":_vm.readonly,"tabindex":"0","step":_vm.step,"min":_vm.min,"max":_vm.max},domProps:{"value":_vm._s(_vm.model)},on:{"blur":[_vm.__updateValue,function($event){_vm.$forceUpdate();}],"keydown":[function($event){if(_vm._k($event.keyCode,"up",38)){ return; }_vm.__updateValue($event);},function($event){if(_vm._k($event.keyCode,"down",40)){ return; }_vm.__updateValue($event);},function($event){if(_vm._k($event.keyCode,"enter",13)){ return; }_vm.__updateValue($event);},function($event){if(_vm._k($event.keyCode,"esc",27)){ return; }_vm.model = _vm.value;}],"input":function($event){if($event.target.composing){ return; }_vm.model=_vm._n($event.target.value);}}}),_vm._v(" "),_c('i',{on:{"click":function($event){_vm.__setByOffset(1);}}},[_vm._v("add")])])},staticRenderFns: [],
  props: {
    value: {
      required: true
    },
